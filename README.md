...
복잡한 라이브러리 설치 과정 없이 **Docker의 멀티 스테이지 빌드**를 활용하여, 누구나 동일한 환경에서 고성능 키-값 저장소(LSM-Tree)를 체험할 수 있습니다.
...
빌드가 완료되면 Docker 컨테이너가 자동으로 `main.cpp`의 테스트 코드를 실행합니다. 터미널 로그를 통해 다음 단계별 작동을 확인할 수 있습니다.
...
- **`docker-compose.yml`** : 컨테이너 환경 설정 및 로컬 폴더(`rocksdb_data`)와의 데이터 동기화(Volume)를 정의합니다.
- **`main.cpp`** : C++20 스마트 포인터를 활용한 RocksDB CRUD 실습 코드입니다.
- **`CMakeLists.txt`** : C++ 표준(20) 설정 및 RocksDB 정적 라이브러리 연결을 위한 빌드 스크립트입니다.
- **`rocksdb_data/`** : (자동 생성) RocksDB의 실제 데이터베이스 파일들이 저장되는 물리적 공간입니다.
